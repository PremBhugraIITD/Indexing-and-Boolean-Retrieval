...
int main(int argc, char* argv[]) {
    ...
    if (fs::exists("stopwords.txt")) stopwords = load_stopwords("stopwords.txt");
    ...
    vector<string> terms;
    for (auto &kv : index) terms.push_back(kv.first);
    ...
    for (size_t t = 0; t < terms.size(); t++) {
        ...
        vector<string> docs;
        for (auto &kv : index[term]) docs.push_back(kv.first);
